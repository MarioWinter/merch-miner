...
# EC-20)
#
# PROJ-11 Phase K3 (2026-04-23): auto-apply re-enabled.
# UploadTemplate and DesignProductConfig now share the ``products_config``
# per-product shape (K1), so the default template's list is copied verbatim
# into the new DesignProductConfig row. `product_config_seeded` reports the
# real outcome (True on successful seed, False on every skip path).
# ---------------------------------------------------------------------------


def _make_default_template(workspace, user, **fields):
    """Build a default UploadTemplate with two per-product entries.

    Default seeds: ``standard_tshirt`` + ``hoodie`` entries — matches prior
    test intent (legacy ``product_types=['standard_tshirt', 'hoodie']`` with
    shared ``fit_types`` / ``colors`` / ``marketplaces`` fan-out).
    """
    defaults = dict(
        name='WS Default',
        brand_name='WSBrand',
...
class TestListingConvertAutoApply:
    """Phase K3: seeding re-enabled.

    Covers AC-57..AC-59, EC-17, EC-19, EC-20: default UploadTemplate's
    ``products_config`` is copied verbatim into the new
    ``DesignProductConfig`` when Convert lands a target with a non-null
    design AND no existing config AND a workspace default for the
    marketplace exists.
    """

    def test_global_to_mba_with_default_template_seeds_config(
        ...
        assert resp.status_code == 201, resp.data
        # AC-57: seeded=True when the default was applied.
        assert resp.data['product_config_seeded'] is True

        # DesignProductConfig row was created and matches the template shape.
        seeded = DesignProductConfig.objects.get(
            design=design, marketplace_type='mba',
        )
        assert len(seeded.products_config) == 2
        entries = {e['product_type']: e for e in seeded.products_config}
        assert 'standard_tshirt' in entries
        assert 'hoodie' in entries
        assert entries['standard_tshirt']['colors'] == ['black', 'white']
        assert entries['hoodie']['colors'] == ['black', 'white']
        # Marketplaces copied verbatim (price normalization optional).
        st_markets = {
            m['marketplace']: m
            for m in entries['standard_tshirt']['marketplaces']
        }
        assert 'amazon.com' in st_markets
        assert 'amazon.de' in st_markets
        assert st_markets['amazon.com']['enabled'] is True
        assert st_markets['amazon.de']['enabled'] is False

    def test_seeded_row_is_deep_copy_independent_from_template(
        self, api_client, workspace, user, idea, design, membership,
    ):
        """Mutating the seeded row must not bleed into the template."""
        template = _make_default_template(workspace, user)
        source = _make_listing(
            workspace, idea, design, Listing.MarketplaceType.GLOBAL,
        )
        ...
        # Mutate the seeded row in-place.
        seeded.products_config[0]['colors'].append('navy')
        seeded.products_config[0]['marketplaces'][0]['enabled'] = False
        seeded.save(update_fields=['products_config', 'updated_at'])

        # Template is untouched.
        template.refresh_from_db()
        tpl_entries = {
            e['product_type']: e for e in template.products_config
        }
        assert tpl_entries['standard_tshirt']['colors'] == ['black', 'white']
        assert (
            tpl_entries['standard_tshirt']['marketplaces'][0]['enabled']
            is True
        )

    def test_convert_with_existing_product_config_skips_seed(
        self, api_client, workspace, user, idea, design, membership,
    ):
        # EC-19: existing config is NEVER overwritten.
        _make_default_template(workspace, user)
        ...
        assert resp.status_code == 201
        # EC-19: no seeding performed -> flag stays False.
    ...
    def test_convert_without_default_succeeds_but_skips_seed(
        ...
        # succeeds, no DesignProductConfig created, flag is False.
        source = _make_listing(
            workspace, idea, design, Listing.MarketplaceType.GLOBAL,
        )
        resp = api_client.post(
    ...
    def test_deleting_sole_default_leaves_no_replacement(
        ...
        # template; next Convert must skip seeding.
        ...
        assert resp.status_code == 201
        assert resp.data['product_config_seeded'] is False
        # EC-17: still no seeded config because no default was set.
        assert not DesignProductConfig.objects.filter(
            design=design, marketplace_type='mba',
        ).exists()

    def test_overwrite_path_seeds_when_no_existing_config(
        self, api_client, workspace, user, idea, design, membership,
    ):
        # Overwrite path (target Listing exists, no DesignProductConfig yet)
        # must still run seeding.
        _make_default_template(workspace, user)
        source = _make_listing(
            workspace, idea, design, Listing.MarketplaceType.GLOBAL,
    ...
    def test_overwrite_path_preserves_existing_config(
        self, api_client, workspace, user, idea, design, membership,
    ):
        # Overwrite path + existing DesignProductConfig -> seed skipped,
        # existing row intact.
        _make_default_template(workspace, user)
        existing_cfg = DesignProductConfig.objects.create(
            design=design,
            marketplace_type='mba',
            products_config=[
                {
        ...
        source = _make_listing(
            workspace, idea, design, Listing.MarketplaceType.GLOBAL,
            title='Fresh', description='Fresh Desc',
        )
        _make_listing(
            workspace, idea, design, Listing.MarketplaceType.MBA,
            title='Stale',
        ...
        assert resp.status_code == 200, resp.data
        assert resp.data['product_config_seeded'] is False
        existing_cfg.refresh_from_db()
        assert existing_cfg.products_config[0]['product_type'] == 'tank_top'

    def test_product_config_seeded_flag_always_present(
        self, api_client, workspace, user, idea, design, membership,
    ):
        # Contract: flag is always in the response body (both 200 + 201).
        source = _make_listing(
            workspace, idea, design, Listing.MarketplaceType.GLOBAL,
        )
        resp = api_client.post(
        ...
        # No default template here -> flag must be False.
        assert resp.data['product_config_seeded'] is False

    def test_seeded_config_copies_empty_products_config_noop(
        self, api_client, workspace, user, idea, design, membership,
    ):
        # Default template with empty products_config seeds an empty row —
        # still reports seeded=True because a row WAS created per AC-57.
        UploadTemplate.objects.create(
            workspace=workspace, created_by=user,
            name='Empty Default', brand_name='B',
            products_config=[],
            marketplace_type=UploadTemplate.MarketplaceType.MBA,
            is_default=True,
        )
