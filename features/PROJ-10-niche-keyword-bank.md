...
A curated, per-niche list of SEO/listing keywords. Aggregates keywords from three sources: auto-import from PROJ-6 deep research results, search-term saves from PROJ-7 autocomplete suggestions, and manual entry. The full list can be injected into PROJ-11 listing generation with one click via a keyword selector modal.

## User Stories

1. As a member, I want to see all keywords saved for a niche in one place, so I can review and curate my keyword collection.
2. As a member, when PROJ-6 research completes, I want research keywords auto-imported into the bank, so I don't have to do it manually.
...
5. As a member, I want to save autocomplete suggestions from PROJ-7 searches to the bank, so I can capture keywords I discover during product research.
...
8. PROJ-7 autocomplete suggestion items show a "+ Save" icon; click opens niche selector (defaults to currently active niche if in niche context); saves keyword with `source=amazon_search`.
9. PROJ-11 listing form: "Add from Keyword Bank" button opens modal listing the niche's `NicheKeyword` rows with checkboxes; selected keywords appended to `extra_keywords` on the generate request.
...
## PROJ-7 Integration (Source: amazon_search)

- PROJ-7 autocomplete suggestions UI gets a "+ Save" icon per suggestion row
...
3. `extra_keywords` injection into PROJ-11 backend_keywords exceeds 500 char limit → truncate to fit; warn user with snackbar (handled in PROJ-11).
...
- PROJ-5 (Niche model)
- PROJ-6 (NicheResearch + NicheKeywordAnalysis models — triggers auto-import)
- PROJ-7 (Amazon Product Research — autocomplete save integration)
- PROJ-11 (consumes keyword bank via "Add from Keyword Bank" modal)
