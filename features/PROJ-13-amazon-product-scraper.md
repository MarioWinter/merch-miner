# PROJ-13: Amazon Product Scraper (Scrapy)
...
Replace the n8n + ScraperOps scraping dependency with a Scrapy-based scraper running as a django-rq job. Same `AmazonProduct` and `ProductSearchCache` models (from PROJ-8) — this is a scrape engine swap. No UI changes required.
...
2. Spider runs via `django_rq.enqueue()` (same `worker` service from PROJ-2).
3. `ProductSearchCache.status` updated to completed/failed by the job.
4. PROJ-4 and PROJ-8 endpoints trigger this job instead of n8n webhook when `USE_SCRAPY_SCRAPER=True` env var is set (feature flag for gradual migration).
...
None — internal engine change only. Existing PROJ-8 status polling endpoints unchanged.
...
- PROJ-8 (Amazon Product Research — models and data flow)
- PROJ-4 (Niche Deep Research — shares infrastructure)
- PROJ-2 (Workspace & Membership — worker service)
