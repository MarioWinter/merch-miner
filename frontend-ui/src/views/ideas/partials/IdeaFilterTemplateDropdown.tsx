...
import { useIdeaFilterTemplates } from '../hooks/useFilterTemplates';
import type { UseIdeaFiltersReturn } from '../hooks/useIdeaFilters';
import type { IdeaFilterTemplate } from '../types';
...
export const IdeaFilterTemplateDropdown = ({ filterState }: Props) => {
  ...
  const {
    ...
    updateTemplate,
    deleteTemplate,
  } = useIdeaFilterTemplates(filterState);
  ...
  const activeTemplate = templates.find((tmpl) => tmpl.id === activeTemplateId) ?? null;
  ...
  const handleApply = (template: IdeaFilterTemplate) => {
  ...
  return (
    <>
      <TriggerButton
        ...
        onClick={handleOpen}
        aria-label={t('ideas.filterTemplate.button')}
        aria-haspopup="true"
        aria-expanded={open}
      >
        {buttonLabel}
      ...
      >
        {isLoading && (
          <MenuItem disabled>
            ...
            <Typography variant="body2">{t('ideas.filterTemplate.loading')}</Typography>
          </MenuItem>
        )}
        ...
        {!isLoading && templates.map((template) => (
          ...
          >
            ...
            <Tooltip title={t('ideas.filterTemplate.delete')} placement="right">
              <span>
                <DeleteBtn
                  size="small"
                  aria-label={t('ideas.filterTemplate.delete')}
        ...
        {activeTemplateId && (
          <MenuItem onClick={() => void handleUpdateCurrent()}>
            ...
            <Typography variant="body2">{t('ideas.filterTemplate.updateCurrent')}</Typography>
          </MenuItem>
        )}
        ...
        {!showSaveInput && (
          <MenuItem onClick={handleSaveClick}>
            ...
            <Typography variant="body2">{t('ideas.filterTemplate.saveFilters')}</Typography>
          </MenuItem>
        )}
        ...
        {showSaveInput && (
          <SaveRow onClick={(e) => e.stopPropagation()}>
            <TextField
              inputRef={inputRef}
              size="small"
              placeholder={t('ideas.filterTemplate.namePlaceholder')}
              value={saveName}
