...
interface ResearchCardContentProps {
  niche: Niche;
  isBusy: boolean;
}
...
const StartButton = styled(Button)(({ theme }) => ({
  ...
  textTransform: 'none',
  '&:hover': {
    background: `linear-gradient(135deg, ${theme.vars.palette.primary.main} 0%, ${theme.vars.palette.primary.dark} 50%, ${theme.vars.palette.primary.main} 100%)`,
    backgroundSize: '200% 100%',
...
export const ResearchCardContent = ({ niche, isBusy }: ResearchCardContentProps) => {
        ...
        <Stack spacing={1.5}>
          <Stack direction="row" spacing={0.75} flexWrap="wrap">
            {researchProgress.marketplace && (
              <Chip
                ...
                variant="outlined"
                sx={{ borderColor: 'divider', height: 24, fontSize: '0.7rem' }}
              />
            )}
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            ...
            <Typography
              variant="caption"
              sx={{
                color: forceRefresh ? 'text.primary' : 'text.secondary',
                cursor: 'pointer',
                userSelect: 'none',
              }}
